...
interface Options {
  //六个hook，用于在First~Forth等页面左滑时，防止Welcome被默认动作拖动
...
export const useSwipe = (
  ...
  const distance = computed(() => {
    //distance返回x、y移动距离，类型自动推断为 ComputedRef (计算结果Ref，下同)
  ...
  const onStart = (e: TouchEvent) => {
    ...
    swiping.value = true; //标记点击开始
      ...
      //为防止遗留上一次的end数据，给start赋值时为end赋值
  ...
  onMounted(() => {
    //onMounted是Vue提供的一个hook(钩子函数)，没有生命周期。Mounted会默认挂到当前组件，随组件加载而挂载，而不是main
    if (!element.value) {
      return;
    }
